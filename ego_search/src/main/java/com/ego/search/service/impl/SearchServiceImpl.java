...
public class SearchServiceImpl implements SearchService {
    @Autowired // Spring boot 2.0 此处有一个 bug
    ...
    public Map<String, Object> search(String q, int page, int size) {
        ...
        for (HighlightEntry<SearchPojo> hlEntity : highlighted) {
            // 非高亮数据，相当于可视化界面中 docs
            SearchPojo searchPojo = hlEntity.getEntity();
            // 把从 solr 中取出的 image 转换为 images
            String image = searchPojo.getImage();
            searchPojo.setImages(image != null && !image.equals("") ? image.split(",") :
                    new String[1]);// 高亮数据，相当于可视化界面中highlighting
            List<HighlightEntry.Highlight> listHL = hlEntity.getHighlights();
            for (HighlightEntry.Highlight highlight : listHL) { // 当前这个对象 item_title 包含高亮数据
                if (highlight.getField().getName().equals("item_title")) { // 替换普通数据
    ...
    public int insert(long[] ids) {
        ...
        for (Long id : ids) {
            ...
            TbItemCat tbItemCat = tbItemCatDubboService.selectById(tbItem.getCid()).get(0);
